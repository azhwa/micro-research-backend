...
const PROMPT_VERSION = "seed-discovery-v1";
...
function compactContext(context: Awaited<ReturnType<typeof getGlobalAiContext>>, input: Required<Pick<SeedDiscoveryInput, "topic" | "category" | "assetType" | "locale">>) {
  ...
  const keywordRows = context.topKeywords
    ...
    .slice(0, 40);
  const fallbackKeywords = keywordRows.length ? keywordRows : context.topKeywords.slice(0, 40);
  return {
    schemaVersion: "seed-discovery-context-1",
    topic: input.topic,
...
export async function createSeedDiscoveryJob(ownerUserId: string, auth: AuthContext, input: SeedDiscoveryInput) {
  ...
  const compact = compactContext(context, { topic, category, assetType, locale });
  const inputHash = createHash("sha256")
    .update(JSON.stringify({ ownerUserId, organizationId: auth.organizationId, promptVersion: PROMPT_VERSION, model, count, compact }))
    .digest("hex");
